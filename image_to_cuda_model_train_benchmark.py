...
def run_model_train(ds: Dataset, batch_size: int, model: nn.Module, cuda_first: bool = True, num_epoches: int = 1, do_warmup: bool = True):
    model = model.cuda()
    optimizer = optim.SGD(model.parameters(), lr=0.001, momentum=0.9)
    scaler = GradScaler()

    dl = DataLoader(ds, batch_size=batch_size, num_workers=min(batch_size, 8), pin_memory=True)
    criterion = nn.CrossEntropyLoss()
    if do_warmup:
        print("warming up... ")
        run_model_train(ds, batch_size, model, cuda_first, num_epoches=4, do_warmup=False)
        print("done!")
    ...
    for name, model_func in models.items():
        for img_size in IMAGE_SIZES:
            for batch_size in BATCH_SIZES:
                ...
                prof = run_model_train(ds, batch_size, model, cuda_first=True, num_epoches=NUM_EPOCHES)
                    ...
                    logger.info(
                        {
                            ...
                            "name": key.key,
                            "img_size": img_size,
                            "time": (key.cpu_time_total + key.cuda_time_total) / 1000 / NUM_EPOCHES ,
                            "img/ms": (key.cpu_time_total + key.cuda_time_total) / 1000 / NUM_EPOCHES / ds.num_images,
                        }
                    )
                benchmark =  f"{name}_{batch_size}_{img_size}_cuda_first=False"
                prof = run_model_train(ds, batch_size, model, cuda_first=False, num_epoches=NUM_EPOCHES)
                keys = prof.key_averages()
                my_keys = list(filter(lambda e: e.key in ["run_model_train"], keys))
                for key in my_keys:
                    logger.info(
